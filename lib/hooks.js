import { db, auth, getItemTypeData, getItemTierData, getUser } from './firebase'
...
export function useItemData(id) {
    ...
    useEffect(() => {
        ...
        unsubscribe = onSnapshot(doc(db, 'items', id), async function fetchData(doc) {
            setItem(doc.data());
            if(doc.data()){setItemTier(await getItemTierData(doc.data().tier.id))}
            if(doc.data()){setItemType(await getItemTypeData(doc.data().type.id))}
