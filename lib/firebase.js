...
export async function buyItem(userUid, listingId, itemId, price) {
...
export async function sellItem(userUid, listingId, itemId, price, username) {
    addItemToMarket(itemId, price, username)
...
export async function removeItemFromMarket(listingId) { 
    ...
    await deleteDoc(doc(db, 'wts', listingId))
    console.log(listingId)
}

export async function addItemToMarket(itemId, price, username) { 
    await addDoc(collection(db, 'wts'), {
        item: doc(db, 'items/' + itemId),
        lister: username,
        price: price,
        timestamp: Timestamp.now()
