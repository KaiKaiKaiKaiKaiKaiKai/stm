...
import { transColor } from '../components/Color'
...
function ListForm({ trans }) {
  ...
  return (
    <form className="p-4 bg-zinc-800">
      ...
      <br />
      <div onClick={ submitForm } className={`rounded-sm text-center font-semibold cursor-pointer bg-${transColor(trans)} text-zinc-800 text-base p-2`}>List</div>
    </form>
  )
}
